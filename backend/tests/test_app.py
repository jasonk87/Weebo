...
def test_chat_success_mocked(client, mocker):
    mocker.patch('backend.app.DEFAULT_USER_ID', 1)
    ...
    with client:
        with flask_app.app_context():
            history = db.get_session_history(db.get_db(), 'test-session')
...
def test_save_fact_tool(client, mocker):
    mocker.patch('backend.app.DEFAULT_USER_ID', 1)
    mock_post = mocker.patch('requests.post')
    tool_call_response = [{"type": "tool_call", "tool_name": "save_fact", "arguments": {"fact_key": "user_city", "fact_content": "London"}}]
    mock_post.return_value = MockResponse(tool_call_response, 200)
